...
def read_assembly_reads(read_fn):
    ...
    output_reads += [_[1][::1] for _ in reads]       # 03. add 2nd read in each pair
...
def simple_de_bruijn(sequence_reads, k):
    ...
    de_bruijn_graph = {key: {val for val in de_bruijn_counter[key] if de_bruijn_counter[key][val] > 3}          # 01. >1 -> >2
    ...
    for key in de_bruijn_counter:
        for k in de_bruijn_counter[key]:
            out_degrees[key] += de_bruijn_counter[key][k]

    in_degrees = defaultdict(int)
    for value in de_bruijn_counter.values():
        for kmer in value:
            in_degrees[kmer] += value[kmer]


    return de_bruijn_graph, in_degrees, out_degrees     # 02. interface


def de_bruijn_reassemble(de_bruijn_graph, in_degrees, out_degrees):     # 02. interface
    """
    ...
    while True:
        ...
        good_starts = [k for k in de_bruijn_graph if de_bruijn_graph[k] and out_degrees[k] > in_degrees[k]]  # 02. if de_bruijn_graph[k] -> if out_degree[k] > in_degree[k]
        if len(good_starts) == 0:                   # 02. add these 2 lines
...
if __name__ == "__main__":
    ...
    input_folder = '../data/hw3all_A_3_chr_1/'.format(chr_name)
    reads_fn = join(input_folder, 'reads_{}.txt'.format(chr_name))
    reads = read_assembly_reads(reads_fn)
    db_graph, in_degrees, out_degrees = simple_de_bruijn(reads, 30)     # 02. interface     # 03. 25 -> 35
    for k in db_graph.keys()[:40]:
        print k, db_graph[k]

    output = de_bruijn_reassemble(db_graph, in_degrees, out_degrees)     # 02. interface
